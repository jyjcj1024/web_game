...
window.onload = () => { puzzleContainer = document.getElementById('puzzle-container'); showImageSource('firebase'); loadImages(); };
...
async function loadImages() {
  ...
  const container = document.getElementById('thumbnailContainer'); container.innerHTML = 'Loading...';
  try {
    const snapshot = await db.collection('images').where('category', '==', category).get();
    snapshot.forEach(doc => imageList.push(doc.data().url)); displayThumbnails(imageList);
  } catch (error) { console.error(error); container.innerHTML = '이미지를 불러올 수 없습니다.'; }
}
function displayThumbnails(images) {
  ...
  images.forEach(url => {
    ...
    img.onclick = (e) => { document.querySelectorAll('.thumbnail').forEach(t => t.classList.remove('selected')); e.target.classList.add('selected'); startPuzzleWithImage(url); };
    container.appendChild(img);
  });
}
document.getElementById('uploadImage').addEventListener('change', (e) => {
  const file = e.target.files[0]; if (!file) return;
  const reader = new FileReader(); reader.onload = (event) => startPuzzleWithImage(event.target.result); reader.readAsDataURL(file);
});
function startWithExternalImage() {
  const url = document.getElementById('imageUrl').value.trim();
  if (url) { startPuzzleWithImage(url); } else { alert(i18next.t('alert_noUrl')); }
}
function startPuzzleWithImage(imageUrl) {
  const pieces = parseInt(document.getElementById('pieces').value); const img = new Image(); img.crossOrigin = "Anonymous";
  img.onload = () => {
    isGameRunning = true; resetTimer(); startTimer(); const aspectRatio = img.width / img.height;
    const containerWidth = Math.min(window.innerWidth * 0.9, 800);
    puzzleContainer.style.width = `${containerWidth}px`; puzzleContainer.style.height = `${containerWidth / aspectRatio}px`;
    pieceWidth = containerWidth / pieces; pieceHeight = (containerWidth / aspectRatio) / pieces;
    puzzleContainer.style.gridTemplateColumns = `repeat(${pieces}, 1fr)`; puzzleContainer.style.gridTemplateRows = `repeat(${pieces}, 1fr)`;
    createPuzzlePieces(img, pieces); shuffleAndDisplayPieces();
  };
  img.onerror = () => { alert(i18next.t('alert_cantLoadImage')); }; img.src = imageUrl;
}
function createPuzzlePieces(img, pieces) {
  ...
  for (let y = 0; y < pieces; y++) {
    for (let x = 0; x < pieces; x++) {
      ...
      piece.dataset.correctX = x; piece.dataset.correctY = y; puzzlePieces.push(piece); addDragListeners(piece);
    }
  }
...
function shuffleAndDisplayPieces() {
  ...
  puzzlePieces.forEach((piece) => {
    const randomX = Math.random() * (window.innerWidth - pieceWidth * 2); const randomY = containerRect.bottom + 20 + Math.random() * 50;
    piece.style.top = `${randomY}px`; piece.style.left = `${randomX}px`; document.body.appendChild(piece);
  });
}
function addDragListeners(piece) {
  let offsetX, offsetY;
  function onDown(e) {
    e.preventDefault(); piece.classList.add('dragging'); document.body.appendChild(piece);
  ...
  function onUp() { if (!piece.classList.contains('dragging')) return; piece.classList.remove('dragging'); snapToGrid(piece);
...
function snapToGrid(piece) {
  ...
  if (pieceCenterX > containerRect.left && pieceCenterX < containerRect.right && pieceCenterY > containerRect.top && pieceCenterY < containerRect.bottom) {
    ...
    piece.style.left = `${containerRect.left + gridX * pieceWidth}px`; piece.style.top = `${containerRect.top + gridY * pieceHeight}px`;
    checkCompletion();
  }
}
function checkCompletion() {
  ...
  for (const piece of puzzlePieces) {
    const pieceRect = piece.getBoundingClientRect(); const containerRect = puzzleContainer.getBoundingClientRect();
    const currentX = Math.round((pieceRect.left - containerRect.left) / pieceWidth); const currentY = Math.round((pieceRect.top - containerRect.top) / pieceHeight);
    if (currentX.toString() !== piece.dataset.correctX || currentY.toString() !== piece.dataset.correctY) { return; }
  }
...
function closeWinModal() {
  document.getElementById('win-modal').classList.add('hidden'); puzzlePieces.forEach(p => p.remove()); puzzlePieces = [];
  puzzleContainer.innerHTML = ''; puzzleContainer.style.width = '0'; puzzleContainer.style.height = '0';
  isGameRunning = false; resetTimer();
}let t;
